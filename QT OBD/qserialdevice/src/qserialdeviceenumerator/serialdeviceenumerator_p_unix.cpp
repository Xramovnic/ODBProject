...
void SerialDeviceEnumeratorPrivate::_q_processWatcher()
{
    ...
    FD_ZERO(&fds);
    FD_SET(this->udev_socket, &fds);
    tv.tv_sec = 0;
    tv.tv_usec = 0;

    ret = select(this->udev_socket+1, &fds, NULL, NULL, &tv);

    if (ret > 0 && FD_ISSET(this->udev_socket, &fds)) {
        struct udev_device *dev;
        dev = udev_monitor_receive_device(this->udev_monitor);
